...
public:
        ...
        , m_kayframe_value_provider(property.get_keyframes())
    {
        ...
        observe(&m_expression_value_provider);
    }
    ...
    void update(UpdateContext &context) override
    {
        ...
        } else if (m_property.value_type() == PropertyAnimationType::e_Constant) {
            if (m_property.expression_enabled()
                && !m_property.get_expression(context.time).empty()) {
    ...
    CRefValueType value() const override
    {
        ...
        if (m_property.value_type() == PropertyAnimationType::e_Constant
                && (!m_property.expression_enabled())
            || m_property.get_expression(0).empty()) {
