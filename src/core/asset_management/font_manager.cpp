...
struct FontInfo
{
    FontInfo(SkTypeface *t, DatabaseAssetId id, int name_id)
        : typeface(t)
        , db_id(id)
        , style_name_index(name_id)
    {}
...
void FontManager::load_system_fonts()
{
    if (!m_d->system_fonts_loaded) {
        const auto &sys_font_mgr = alive::ResourceManager::system_font_manager();
        for (size_t i = 0; i < sys_font_mgr.countFamilies(); ++i) {
            ...
            for (size_t j = 0; j < styleset->count(); ++j) {
                add_typeface(styleset->createTypeface(j).release(), k_invalid_db_id);
            }
        }
...
void FontManager::add_typeface(SkTypeface *typeface,
                               ...
                               const std::string &style)
{
    if (typeface) {
        ...
        ALIVE_CORE_TRACE("Adding Font Family {}, Style {}", family_name, style_name);
    }
}
